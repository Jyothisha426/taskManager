...
const TaskForm = ({ addTask }) => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    addTask({ title, description, priority, deadline, category });
    setTitle('');
    setDescription('');
    setPriority('Low');
    setDeadline('');
  ...
  return (
    <form className="task-form" onSubmit={handleSubmit}>
      ...
      />
      <select value={priority} onChange={(e) => setPriority(e.target.value)} className='priority-input'>
      ...
      />
      <select value={category} onChange={(e) => setCategory(e.target.value)} className='category-input'>
